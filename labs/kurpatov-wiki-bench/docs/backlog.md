...
## Top-10 by ICE (текущий ranking, после F1 done)

| # | ID  | title                                       | ICE | status   | spec                                  |
|---| --- | ------------------------------------------- | ---:| -------- | ------------------------------------- |
| 1 | A8  | reproduce T3 with max_tokens=32768 patch    | 400 | triaged  | —                                     |
| 2 | A4  | vLLM guided JSON decoding                   | 336 | backlog  | —                                     |
| 3 | A1  | bump max_completion_tokens до 32 768        | 300 | triaged  | —                                     |
| 4 | F4  | GPT-4o pairwise judge                       | 210 | backlog  | —                                     |
| 5 | A5  | lm-format-enforcer                          | 210 | backlog  | —                                     |
| 6 | B3  | gpt-oss-20b на T1                           | 200 | backlog  | —                                     |
| 7 | A2  | hermes parser swap                          | 180 | backlog  | —                                     |
| 8 | A3  | tool_choice="required"                      | 160 | backlog  | —                                     |
| 9 | D2  | skill v3 — terminal-only writes (defense)   | 128 | backlog  | —                                     |
|10 | D3  | skill v4 — local write + git (defense)      | 112 | backlog  | —                                     |

**Done:**
- [F1](experiments/F1.md) (microbench L*) — ICE 648, **REFUTED**: модель Qwen3.6-27B-FP8 в isolated-call setup проходит JSON-tool-args на всех L от 1 KB до 49 KB (pass-rate=1.00). `L*_isolated ≥ 49 152 bytes`. Корень T3 crash'а — не в архитектурном лимите модели. Top подозреваемый: max_completion_tokens cap в OpenHands LLMConfig.
