"""
D7-rev4-v2 production orchestrator — TaskToolSet variant.

Same architecture as D7-rev4 but using TaskToolSet (current API) instead
of deprecated DelegateTool. Validated on synth (Step 5d-rev v2):
  4/4 sources verified=ok, claims=26, REPEATED=8, CF=2,
  concepts=11, canonical-template OK, top events=12.

Why migration from DelegateTool to TaskToolSet:
  D7-rev4 attempt #1 with DelegateTool spawn-once-reuse + full lecture
  transcript per delegate caused production-scale context bloat
  (state.events accumulation → 100K+ tokens per sub-agent call,
  ~5 min/claim, projected 14 hours total → aborted).

  TaskToolSet's `task(description, prompt, subagent_type)` creates a
  FRESH sub-agent per call — no state accumulation, no spawn cap,
  ~5K tokens/call instead of 250K.

Cyrillic paths preserved literally. Wikipedia rate-limit mitigation
via identifiable User-Agent + selective fact-check (D7-rev4 spec).

Usage:
  python3 orchestrator/run-d7-rev4-v2.py
Env:
  LLM_BASE_URL, LLM_API_KEY, LLM_MODEL — vLLM config
  GITHUB_TOKEN — for git push
  D7_REV4_V2_WORKDIR (optional) — default /tmp/d7-rev4-v2-prod
"""
import json
import os
import shutil
...
WORKDIR = Path(os.environ.get("D7_REV4_V2_WORKDIR", "/tmp/d7-rev4-v2-prod"))
...
Note: file_editor requires ABSOLUTE paths. Prefix paths with the workspace
root (terminal `pwd` shows it).
"""

SOURCE_AUTHOR_PROMPT = f"""\
You are source-author. Process ONE Курпатов lecture into a skill-v2 source.md.
Use the `task` tool (NOT `task_tool_set` — registry name is `task_tool_set`
but the exposed function is named `task`) to invoke sub-agents. Each
`task(...)` call is FRESH context (no state accumulation).
...
URL goes inline in `## Claims` after the marker (if any).
...
mkdir -p the parent dir before file_editor. file_editor requires ABSOLUTE
paths — prefix with workspace root (`pwd` in terminal shows it).
...
def setup_workspace():
    ...
    branch = f"experiment/D7-rev4-v2-{date.today().isoformat()}-{served}"
...
def write_bench_report(state, branch, partial=False):
    ...
    lines = [f"# bench report — D7-rev4-v2 (TaskToolSet) {'(partial)' if partial else ''}",
             "", f"branch: `{branch}`",
...
def main():
    ...
    print("D7-rev4-v2 production orchestrator (TaskToolSet)", file=sys.stderr)
    print("=" * 70, file=sys.stderr)
    ...
    master = (
        ...
        "  2. If reply starts with 'failed:' — finish with 'STOPPED at N=<N>: <reply>'.\n"
        "  3. Else proceed to next source.\n"
        "\nInputs:\n" + "\n".join(inputs_lines) + "\n"
