...
against an open-weight model served by the local `phase-b-business-architecture/org-units/wiki-compiler/`
...
(specifically, `phase-b-business-architecture/org-units/wiki-compiler/`).
...
- Not a model server. We consume the vLLM endpoint that
  `phase-b-business-architecture/org-units/wiki-compiler/` exposes; we do not start or stop vLLM.
...
`phase-b-business-architecture/org-units/wiki-compiler/` must be up; `forge/rl-2048/` is irrelevant.
...
vLLM HF cache (configured in `phase-b-business-architecture/org-units/wiki-compiler/`):
