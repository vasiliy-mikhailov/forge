...
`${STORAGE_ROOT}/labs/wiki-ingest/vault/` is unchanged.
...
On the server, `${STORAGE_ROOT}/labs/wiki-ingest/vault/raw/` is a git
...
## Follow-ups
- Rename the on-disk dir `${STORAGE_ROOT}/labs/wiki-ingest/vault/` and the
