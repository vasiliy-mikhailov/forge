...
| Source-author   | [`source-author.md`](source-author.md)     | (transitive — `phase-c-…/wiki-bench/tests/synthetic/test_source_coordinator_*.py`) | Operational role: compile one raw transcript into one `source.md` per activation. Filled by an OpenHands SDK agent inside the wiki-bench harness. |
| Concept-curator | [`concept-curator.md`](concept-curator.md) | (transitive — same)                                                                       | Operational role: maintain `data/concepts/` graph (create on introduce, update on touch, keep `concept-index.json` consistent). Sub-Conversation spawned from source-author. |
