...
1. **Measurable motivation chain stated in the role md.** Every role file
   ...
   New role files include this chain in a `## Measurable motivation chain`
...
2. **Reward function per test case.** Each AAA case in a test md
   ...
   - **Motivation reference** — which Outcome the score
     measures (cite the role's measurable motivation chain).
...
- *Motivation gets typed traceability.* Each role's tests cite
  the Measurable motivation chain (Driver → Goal → Outcome → Capability →
...
- (Sibling commit) `phase-b-business-architecture/roles/auditor.md`
  + `wiki-pm.md` gain a `## Measurable motivation chain` section citing the
...
- [ADR 0014](0014-archimate-across-all-layers.md) — ArchiMate 4
  vocabulary. The Measurable motivation chain referenced in §1 of this
