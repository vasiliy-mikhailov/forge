...
- **P14** — role files have a `## Motivation chain` section.
...
- **ADR 0015 dec 1** — motivation chain required per role.
...
> ArchiMate aspect) MUST cite a motivation chain (Driver → Goal
...
> file contains either `## Motivation chain` section OR a
...
Motivation chain in the same commit so P24 lands clean.
...
phase has its own `## Motivation chain` (P24) AND every
...
- **Minus**: more prose per artifact (the motivation chain is
...
- A new artifact type added to forge without a motivation chain
...
- **Make Motivation chain a per-file YAML frontmatter
