...
[amendment](../../../../SOLUTION-ARCHITECTURE.md).
## Why
sub-A landed the parallel runner inside the container; sub-B
...
referenced by `_execute_submission` for the dev path
will move that to Docker too).
## Contract
`main(model_id, seeds=..., config=..., canonical_scorer=None)`:
- `canonical_scorer` is an injection point. When `None`, `main()` constructs a default
