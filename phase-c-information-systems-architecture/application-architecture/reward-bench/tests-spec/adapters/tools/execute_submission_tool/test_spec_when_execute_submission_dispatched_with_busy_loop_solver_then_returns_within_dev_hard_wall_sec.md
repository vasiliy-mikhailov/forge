...
isolation (ADR 0006 Layer 2), not soft daemon-thread timeouts.

## The bug this test catches

Pre-cycle-128, `_execute_submission` called `score_submission`
in-process per ADR 0006 Layer 1 (daemon-thread per-game timeout).
...
Per cycle 122, this is the exact gap a `@live` test with an
...
| **live**   | `ExecuteSubmissionTool` + real `_execute_submission` + DockerCanonicalScorer (cycle 128) | dev_hard_wall_sec=30, 5 dev seeds, busy-loop solver |
