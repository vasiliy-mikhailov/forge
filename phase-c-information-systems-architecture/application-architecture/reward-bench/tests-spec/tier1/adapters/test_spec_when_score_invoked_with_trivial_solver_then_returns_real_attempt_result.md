...
[cycle 122](../../../../../AGENTS.md#three-runtimes-two-scales-of-src_spec--unit--live--production).
...
This is the test cycle 122 says every runtime-boundary Port MUST have.
Without it, image-missing / runner-broken / Docker-flag-wrong bugs
ship silently to the canonical bench — exactly what cycle 105
demonstrated.

## Bug found at RED

Writing this test surfaced a real production bug introduced in
cycle 105 sub-A: `tasks/2048/runner_canonical.py` line 137 called
...
between cycle 105 and cycle 123. Every canonical scoring trial since
cycle 105's image bump emitted 20 × `walltime_exceeded` sentinels
because the in-container runner crashed with `AttributeError` before
writing `result.json` (the silent-sentinel path that cycle 121
...
the sentinel path per cycle 121's line between infra and
