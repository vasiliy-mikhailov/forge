...
def test_when_run_loop_produces_submission_then_solver_move_returns_one_of_wasd(
    ...
    run_loop(workspace, env_dir, tasks_dir, vllm_base_url, vllm_api_key, max_iters=15, max_tokens=4096)
...
def test_when_execute_submission_called_then_workspace_submission_py_is_not_written(monkeypatch, tmp_path):
    """Pins §7.5 cleanup: _execute_submission no longer writes
    workspace/submission.py for module loading. Body is compiled in
    memory; workspace stays empty unless other paths write to it."""
    # Arrange
    from src.tier1 import agent_loop as al
    from src.tier1.entities.attempt_result import AttemptResult
    ...
    # Assert
    assert not (workspace / 'submission.py').exists(), (
        '§7.5: _execute_submission must not write submission.py to workspace'
    )
