"""Legacy stage-1 agent loop — the BLESSED RUNNER per ADR 0007.

Promoted verbatim from _bak/bin/agent_loop.py in cycle 47. Until the
src/tier1/agent_loop.py regression is bisected (cycle 40 reproduced
~40 percent score gap vs this loop on the same model + vLLM), the
per-model bench test uses THIS module as a subprocess.

When src/tier1/agent_loop.py reaches parity, ADR 0007 is superseded
and this module can be removed. Until then it is FROZEN — no
behaviour-changing edits — so the score parity is preserved.
