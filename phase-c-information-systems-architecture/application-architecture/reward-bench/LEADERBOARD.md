...
| candidate-tier1: nemotron-3-super-120b-nvfp4 | NVIDIA Nemotron 3 Super 120B-A12B (NVFP4, MoE) — **rejected**: required `--max-model-len 16384` + `--max-num-seqs 64` (Mamba-hybrid arch with limited Mamba cache blocks at 75 GB). Same 16K context overflow as Devstral-123B (model is also reasoning-heavy with `<think>` tags inflating per-turn token spend). 4 KB submission written but corrupted by `---` separator leak — SyntaxError on import. | — | — | — | — | — | — |
