...
by delegating plateau judgment to the bench LLM under test, per [SOLUTION-ARCHITECTURE](../../../../SOLUTION-ARCHITECTURE.md)
(same model as bench) + [SOLUTION-ARCHITECTURE](../../../../SOLUTION-ARCHITECTURE.md)
...
`judge(sweep)` flow:
1. **Render**: build a JSON-format prompt from the sweep tuples's prescribed schema. The prompt asks the model to reply
