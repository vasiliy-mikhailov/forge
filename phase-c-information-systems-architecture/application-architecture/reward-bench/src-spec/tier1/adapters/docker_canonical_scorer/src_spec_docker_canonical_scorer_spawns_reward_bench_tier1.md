...
[`DockerCanonicalScorer`](../../../../src/tier1/adapters/docker_canonical_scorer.py) / [SOLUTION-ARCHITECTURE](../../../../SOLUTION-ARCHITECTURE.md)
implementation. Spawns the `reward-bench-tier1:0.4` container per attempt.
## Contract
Constructed with:
- `image` (default `reward-bench-tier1:0.4`)
- `cpus` (default `cpu_count_port.cpu_count() / 2` — host-side cap)
