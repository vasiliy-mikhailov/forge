...
class SourceCoordinator:
    ...
    def _prompt_classify_batch(
        ...
        prompt = (
            ...
            "  category: short kebab-case thematic slug IN THE SAME LANGUAGE\n"
            "    as the claim. If the claim is Russian, the slug is Russian\n"
            "    (e.g. 'академическая-фрагментация',\n"
            "    'комплекс-неполноценности'). Lowercase. Hyphens between\n"
            "    words. No spaces, no quotes, no parentheses, no slashes.\n"
    ...
    def _prompt_fact_check(self, claim: dict) -> str:
        return (
            ...
            "  notes: ONE sentence in the SAME LANGUAGE as the CLAIM "
            "(if claim is Russian, notes are Russian). No multi-step "
            "reasoning, no 'wait, let me re-evaluate' spirals, no nested "
            "quotes, no escaped backslashes. If you find yourself "
            "second-guessing, pick a marker and write one sentence.\n\n"
