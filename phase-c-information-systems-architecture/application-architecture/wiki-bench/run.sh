#!/usr/bin/env bash
# wiki-bench/run.sh — one bench run, sandboxed in Docker.
#
...
#   - Inference is delegated to the wiki-compiler lab
...
#   - Compiler lab up (`make wiki-compiler` from forge root).
...
: "${EXPERIMENTS_ROOT:=$STORAGE_ROOT/labs/wiki-bench/experiments}"
...
if [[ -z "$served" ]]; then
  ...
  echo "       Bring up the compiler lab first: make wiki-compiler" >&2
  exit 3
fi
...
if [[ -n "$ACTIVE_ID" ]]; then
  expected=$(python3 - "$FORGE_ROOT/labs/wiki-compiler/configs/models.yml" "$ACTIVE_ID" <<'PY' 2>/dev/null || true
import sys, yaml
...
PY
  ...
  if [[ -n "$expected" && "$served" != "$expected" ]]; then
    ...
    echo "       Re-run: make kurpatov-wiki-compiler-down && make wiki-compiler" >&2
    exit 3
  fi
fi
