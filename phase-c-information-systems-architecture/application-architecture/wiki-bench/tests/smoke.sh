...
#
# Bench is co-runnable with wiki-compiler (compiler is its
...
[[ -f "$FORGE_ROOT/labs/wiki-bench/.env" ]] && set -a && \
  . "$FORGE_ROOT/labs/wiki-bench/.env" && set +a
...
bench_dir="$FORGE_ROOT/labs/wiki-bench"
...
if [[ -z "$served" ]]; then
  fail "compiler endpoint ${INFERENCE_BASE_URL} unreachable (is wiki-compiler lab up?)"
