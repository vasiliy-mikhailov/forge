# Research Backlog — wiki-compiler
...
## Top-10 by ICE (текущий ranking, после F1 + A8 done)

| # | ID  | title                                       | ICE | status   | spec                                  |
|---| --- | ------------------------------------------- | ---:| -------- | ------------------------------------- |
| 1 | F4  | GPT-4o pairwise judge (quality vs Opus)     | 280 | triaged  | —                                     |
| 2 | B3  | gpt-oss-20b на T3 (с 128K context)          | 250 | triaged  | —                                     |
| 3 | B4  | Devstral-Small-2-24B на T3                  | 200 | triaged  | —                                     |
| 4 | B5  | Mistral-Small-3.2-24B на T3                 | 200 | triaged  | —                                     |
| 5 | B6  | Nemotron-Super-49B-v1.5-FP8 на T3           | 180 | triaged  | —                                     |
| 6 | B7  | Llama-3.3-70B-NVFP4 на T3                   | 150 | backlog  | —                                     |
| 7 | A4  | vLLM guided JSON decoding (defense)         | 100 | backlog  | —                                     |
...
- **A8** (T3 reproduce with 128K context) — ICE 400, **PASSED 2026-04-25**: бамп `max_model_len: 65536 → 131072` (YaRN factor 4.0) заставил Qwen3.6-27B-FP8 + skill v1 пройти T3 source 000 atomic commit+push на `bench/2026-04-25-qwen3.6-27b-fp8`. Корень T3 crash'а — `max_model_len` budget. Pass partial (source 136 vs Opus 216 строк; 6 концептов vs 32) — quality regression; основной механизм пробит.
- **A1** (max_completion_tokens bump) — ICE 300, **DROPPED**: после A8 не нужен. Default OpenHands max_completion_tokens хватает; реальный bottleneck был в max_model_len (контекстное окно), не в output limit.
...
**ICE 300 (6 · 5 · 10) · triaged ← ↑ после F1**

В OpenHands LLMConfig (передаётся через `bench/run.sh`) поднять
`max_completion_tokens` с дефолта (~4096) до 32 768. Confidence ↑ после F1
microbench: модель легко пишет 49 KB JSON-args в isolated call, а в T3
крашится на ~285 chars — это паттерн усечения по token budget'у, не по
архитектуре модели. T3 prompt ~50K input + body 16K tokens требует
≥ 16K на output; default OpenHands значительно меньше.
*Опровергнуто, если в логах T3 run'а `stop_reason ≠ length`, либо при
max=32K тот же `Unterminated string`.*

#### A8 — reproduce T3 with `max_completion_tokens=32768` patch

**ICE 400 (10 · 5 · 8) · triaged**

Главный pivot после F1. Патчим `bench/run.sh` (или OpenHands LLMConfig
override через env-var) на `max_completion_tokens=32768`, перезапускаем
тот же T3 single-source бэнч (Qwen3.6-27B-FP8, skill v1, source 000).
Если проходит до commit'а — **прямое подтверждение, что T3 crash был
artifact'ом max_tokens cap'а**. North Star достигнут одной строкой
конфига; остальной backlog (D-cluster, B-cluster) становится опционным.
*Опровергнуто, если T3 даёт `Unterminated string` на том же или другом
file_editor вызове даже при max=32K. Тогда — H10 (context starvation
другого свойства, не token budget).*

#### A2 — swap parser `qwen3_xml` → `hermes`

**ICE 180 (6 · 3 · 10) · backlog**

В `phase-c-information-systems-architecture/application-architecture/wiki-compiler/docker-compose.yml` — флаг
...
**ICE 90 (10 · 3 · 3) · backlog ← ↓ после F1 (defense-in-depth)**
...
**ICE 128 (8 · 4 · 4) · backlog ← ↓ после F1 (defense-in-depth)**
...
**ICE 112 (7 · 4 · 4) · backlog ← ↓ после F1 (defense-in-depth)**
...
Все source модуля 005 усечены до 30 KB на этапе `wiki-ingest`.
...
#### F1 — microbench `L*` (DONE 2026-04-25)

**ICE 648 (8 · 9 · 9) · done — REFUTED — [experiments/F1.md](experiments/F1.md)**

T1 синтетический тест: 10 длин Cyrillic body × (2-10) trials. **Результат:
pass-rate=1.00 на всех L от 1 KB до 49 KB. `L*_isolated ≥ 49 152 bytes`.**
Гипотеза «модель имеет JSON-truncation bug при длинных Cyrillic args»
**REFUTED** (criterion 2 falsifier'а: pass-rate не убывает с ростом L).
Pivot: T3 crash причина — не модель, а max_completion_tokens cap →
A1 / A8 поднимаются в top.
...
| F1  | модель имеет JSON-truncation bug при длинных Cyrillic args (length-dependent)           | refuted 2026-04-25: T1 microbench Qwen3.6-27B-FP8 → pass-rate=1.00 на L от 1 KB до 49 KB. См. [experiments/F1.md](experiments/F1.md). T3 crash имеет другую причину. |
| H2  | Qwen3.6 family-specific tool-mode bug (нужно тестить Qwen3.5/Qwen3-32B)                | refuted косвенно через A8: проблема в context budget, не в семействе. Тестить Qwen3.5/3.0 на T3 более не актуально.                                                  |
