...
| R-B-compact-restore      | A: TTS + Architect-velocity | Reading speed (forward) + Concept-graph quality (backward)  | No algorithm — readers either consume the full lecture (88 min for source A) or a hand-edited synopsis (compression ratio 0.34, no formal recall guarantee). Algorithmic round-trip not measured. | Two-way `compact()` / `restore()` algorithm shipped from wiki-bench. Trip-quality `min(fwd_recall, bwd_recall) × (1 - ratio)` ≥ 0.50 on lecture A at L1+L2+L3 (Air-strip + cross-source dedup + concept-graph link-out); ≥ 60% saved-time vs the raw lecture; voice preservation eye-read PASS. | K2 in flight (Phase F): [`phase-f-migration-planning/experiments/K2-compact-restore.md`](../phase-f-migration-planning/experiments/K2-compact-restore.md) | OPEN   |
